...
- 既存コードに対しても積極的にリファクタリングを検討し、ユーザーへ提案する (勝手にやらないこと。ユーザーの同意を必須とする)
...
- コードを書いた後は必ず整形およびテストコードの実行をする
  - Lintエラー・テスト失敗・コンパイル時警告などは、その時実施中の作業に無関係のものであっても一緒に対応する
  - ドキュメント（`.md`ファイル）を変更した場合は`mise run lint-docs`を実行してtextlint/markdownlint/prettierのチェックを通すこと（`node_modules`がなければ`pnpm install`を先に実行）
  - 全チェックの一括実行: `mise run lint`
...
- CP932変換には `WideCharToMultiByte(CP_ACP, ...)` を使う
